...
def trigger_retraining_via_api():
    """Triggers retraining via the FastAPI /trigger_retrain endpoint."""
...
if page_selection == "Dashboard":
    ...
    with col1:
        st.metric(label="API Health", value="Online" if health_data.get('status') == 'ok' else "Error")
    with col2:
    ...
    try:
        from src import preprocessing # Assuming visualizations are generated by preprocessing.py
        (train_images, train_labels), _ = preprocessing.load_and_preprocess_cifar10()
        class_names = ['airplane', 'automobile', 'bird', 'cat', 'deer', 'dog', 'frog', 'horse', 'ship', 'truck']
        ...
        **Insight:** The dataset is beautifully balanced! Each of the 10 classes has a roughly equal number of images. This is fantastic because it means our model won't be biased towards one class, leading to a fair and robust classifier.
        ...
        for i, idx in enumerate(sample_indices):
            with cols[i]:
                st.image(train_images[idx], caption=class_names[train_labels_flat[idx]], width=64)
        st.info("""
        **Insight:** Here's a glimpse of the model's challenge: classifying these tiny 32x32 images. They're pixelated and abstract, but our human brains can still see a car or a dog. Our CNN model must learn to do the same, extracting key features to make sense of these low-resolution visuals.
        ...
        st.error(f"Could not generate visualizations: {e}. Please ensure the Jupyter notebook was run and data is available locally.")
