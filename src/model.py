...
def build_model(input_shape=(32, 32, 3), num_classes=10):
    ...
    model = models.Sequential([
        layers.Conv2D(32, (3, 3), activation='relu', input_shape=input_shape, padding='same'), # Added padding for consistent size
...
def train_model(model, x_train, y_train, x_val, y_val, epochs=50, batch_size=64, model_name='cifar10_cnn_model.h5'):
    """
    ...
    history = model.fit(x_train, y_train,
                        ...
                        validation_data=(x_val, y_val), # Now using the dedicated validation set
                        callbacks=[early_stopping, model_checkpoint],
                        verbose=1)
    print("Model training complete.")
    print(f"Best model saved to: {model_filepath}")
    return history, model_filepath
...
if __name__ == "__main__":
    ...
    # This aligns with real-world practice for final model evaluation.
